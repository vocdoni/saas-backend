...
//
// startTime is the election's on-chain start, which with the CSP's ChainID
// selects the salt derivation (see electionSalt); the salt is stored in the
// session so CompleteBlindSign signs under the same key this attestation used.
//
...
func (c *CSP) PrepareBlindSign(
	token, electionID internal.HexBytes, weight uint64, startTime uint32,
	...
	salt, err := c.electionSalt(electionID, new(big.Int).SetUint64(weight).Bytes(), startTime)
	if err != nil {
		return nil, nil, err
	}
	...
	// which recovers the same integer either way. The salt rides along so the
	// matching sign call blinds under the key this attestation was made over.
	tokenR = r.Bytes()
	if err := c.Storage.SetCSPBlindSession(
		authTokenData.UserID, electionID,
		k.FillBytes(make([]byte, blindScalarLen)), tokenR, salt, 0,
	); err != nil {
		return nil, nil, errors.Join(ErrSign, err)
	}
...
func (c *CSP) CompleteBlindSign(token, electionID, tokenR, blindedMsg internal.HexBytes) (internal.HexBytes, error) {
	...
	// hand the same k out twice. The salt is the one fixed at prepare time, so
	// the blind signature verifies against the same key the attestation did.
	...
	signature, err := c.Signer.SignBlind(session.Salt, blindedMsg, new(big.Int).SetBytes(session.SecretK))
	if err != nil {
		return nil, errors.Join(ErrSign, err)
	}
...
func (c *CSP) VerifyWeightAttestation(electionID, cert internal.HexBytes, weight uint64, startTime uint32) (bool, error) {
	salt, err := c.electionSalt(electionID, new(big.Int).SetUint64(weight).Bytes(), startTime)
	if err != nil {
		return false, err
	}
