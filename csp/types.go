...
const (
	...
	// SignatureTypeEthereum is the standard secp256k1 signature used in
	// Ethereum
	SignatureTypeEthereum = "ecdsa"
	// SignatureTypeSharedKey identifier the shared key (common for all users
	// on the same processId)
	SignatureTypeSharedKey = "sharedkey"
)

// AllSignatures is a helper list that includes all available CSP signature
// schemes.
...
// AuthField is the type used by the Info method for returning the description
// of the authentication steps for the CSP implementation.
type AuthField struct {
	Title string `json:"title"`
	Type  string `json:"type"`
}

// AuthResponse is the type returned by Auth methods on the AuthHandler. If
// success true and AuthToken is nil, authentication process is considered
// finished, and the CSP signature is provided to the user.
type AuthResponse struct {
	// Either the authentication step is success or not
	Success bool `json:"success"`
	// Response can be used by the handler to provide arbitrary data to the
	// client
	Response []string `json:"response"`
	// Only if there is a next step
	AuthToken *uuid.UUID `json:"authToken,omitempty"`
	// TokenR is the random token generated for the client
	TokenR internal.HexBytes `json:"tokenR,omitempty"`
	// Signature is the CSP signature
	Signature internal.HexBytes `json:"signature,omitempty"`
	// Error is used to provide an error message to the client
	Error string `json:"error,omitempty"`
}
