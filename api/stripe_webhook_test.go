...
func TestStripeWebhook(t *testing.T) {
	...
	t.Run("InvoiceErrors", func(*testing.T) {
		...
		{
			s := mockStripeInvoicePayment(orgAddress, date)
			s.SubscriptionDetails.Metadata["address"] = ""
			err := service.HandleEvent(mockStripeEvent(stripeapi.EventTypeInvoicePaymentSucceeded, s))
		...
		{
			s := mockStripeInvoicePayment(orgAddress, date)
			s.SubscriptionDetails.Metadata["address"] = "0x00dead"
			err := service.HandleEvent(mockStripeEvent(stripeapi.EventTypeInvoicePaymentSucceeded, s))
	...
	t.Run("ProductUpdated", func(*testing.T) {
		{
			...
			c.Assert(plan.StartingPrice, qt.Equals, mockEssentialPlan.StartingPrice)
...

		{
			...
			s.Name = "New Name"
			s.DefaultPrice.UnitAmount = int64(1234)
...

		{
			...
			c.Assert(plan.StartingPrice, qt.Equals, int64(1234))
