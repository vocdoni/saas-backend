...
// sendLocalizedMail method sends a localized notification to the email provided.
...
// it does nothing.
func (a *API) sendLocalizedMail(ctx context.Context, to string, mail mailtemplates.LocalizedMailTemplate, data any) error {
	if a.mail != nil {
		ctx, cancel := context.WithTimeout(ctx, time.Second*10)
		defer cancel()
		// check if the email address is valid
