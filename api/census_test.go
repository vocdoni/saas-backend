...
func TestCensus(t *testing.T) {
	...
	censusInfo := &apicommon.CreateCensusRequest{
		...
		AuthFields: db.OrgMemberAuthFields{
			db.OrgMemberAuthFieldsEmail,
			db.OrgMemberAuthFieldsPhone,
		},
	}
	...
	resp, code = testRequest(t, http.MethodPost, adminToken, duplicateMembers, "organizations", orgAddress.String(), "members")
	c.Assert(code, qt.Equals, http.StatusOK, qt.Commentf("response: %s", resp))

	// Create a new group with all members including duplicates
	allMembersResp, code := testRequest(t, http.MethodGet, adminToken, nil, "organizations", orgAddress.String(), "members")
	c.Assert(code, qt.Equals, http.StatusOK)
	...
	duplicateGroupRequest := &apicommon.CreateOrganizationMemberGroupRequest{
		...
		Description: "A group with duplicate email addresses",
		MemberIDs:   allMemberIDs,
	}

	resp, code = testRequest(t, http.MethodPost, adminToken, duplicateGroupRequest, "organizations", orgAddress.String(), "groups")
	c.Assert(code, qt.Equals, http.StatusOK, qt.Commentf("response: %s", resp))
	...
	duplicateCensusInfo := &apicommon.CreateCensusRequest{
		...
		AuthFields: db.OrgMemberAuthFields{
			db.OrgMemberAuthFieldsEmail, // This will have duplicates
		},
	}
	...
	resp, code = testRequest(t, http.MethodPost, adminToken, emptyFieldMember, "organizations", orgAddress.String(), "members")
	c.Assert(code, qt.Equals, http.StatusOK, qt.Commentf("response: %s", resp))

	// Create a group with the empty field member
	updatedMembersResp, code := testRequest(t, http.MethodGet, adminToken, nil, "organizations", orgAddress.String(), "members")
	c.Assert(code, qt.Equals, http.StatusOK)
	...
	resp, code = testRequest(t, http.MethodPost, adminToken, emptyFieldGroupRequest, "organizations", orgAddress.String(), "groups")
	c.Assert(code, qt.Equals, http.StatusOK, qt.Commentf("response: %s", resp))
	...
	// Test 7.1: Try to create a census with email auth field when some members have empty emails (should fail)
	emptyFieldCensusInfo := &apicommon.CreateCensusRequest{
		Type:       db.CensusTypeSMSorMail,
		OrgAddress: orgAddress.String(),
		GroupID:    emptyFieldGroup.ID,
		AuthFields: db.OrgMemberAuthFields{
			db.OrgMemberAuthFieldsEmail, // This will have empty values
		},
	}
	...
	var emptyErrorResponse map[string]interface{}
