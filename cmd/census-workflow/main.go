...
func main() {
	...
	if len(addMembersResp.JobID) > 0 {
		...
		for {
			var jobResp db.BulkOrgMembersStatus
	...
	fmt.Println("\n4. Creating census for the group...")
	...
	err = client.makeRequest(
		...
		apicommon.CreateCensusRequest{
			...
			OrgMemberAuthFields: []db.OrgMemberAuthFields{
				"email",
				"memberNumber",
			},
		}, &createCensusResp,
