...
func generateParticipants(n int) []db.CensusParticipant {
	participants := make([]db.CensusParticipant, n)
	for i := 0; i < n; i++ {
		participants[i] = db.CensusParticipant{
			...
			ParticipantID: fmt.Sprintf("participant_%d", i+1),
...
func main() {
	...
	err = client.makeRequest("POST", fmt.Sprintf("/process/%s", processID), api.CreateProcessRequest{
		PublishedCensusID: base64.StdEncoding.EncodeToString(publishedCensus.ID),
		Metadata:          metadata,
	}, nil)
	if err != nil {
