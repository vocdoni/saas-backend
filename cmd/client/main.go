// package main provides a CLI HTTP client for allowing an organization to interact with backend API.
...
func maskedPhone(phone string, orgAddress common.Address, country string) (string, error) {
	...
	org := &db.Organization{Address: orgAddress, Country: country}

	hash, err := db.NewHashedPhone(phone, org)
	if err != nil {
