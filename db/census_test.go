...
func TestPopulateGroupCensus(t *testing.T) {
	...
	t.Run("InputValidation", func(_ *testing.T) {
		...
		_, err = testDB.PopulateGroupCensus(invalidCensus, someGroupID)
		...
		_, err = testDB.PopulateGroupCensus(nonExistentCensus, someGroupID)
	...
	t.Run("PopulateGroupCensusWithEmptyGroup", func(_ *testing.T) {
		...
		err = testDB.UpdateOrganizationMemberGroup(
			...
			"", "",
			nil, []internal.ObjectID{member1ID}, // Remove the only member
		)
		c.Assert(err, qt.IsNil)
	...
	t.Run("ParticipantHandling", func(_ *testing.T) {
		...
		multiMemberGroup := &OrganizationMemberGroup{
			...
			MemberIDs:   []internal.ObjectID{member1ID, member2ID},
		}
		multiGroupID, err := testDB.CreateOrganizationMemberGroup(multiMemberGroup)
		c.Assert(err, qt.IsNil)

		censusOID3 := internal.NewObjectID()
		census3 := &Census{
			ID:         censusOID3,
			OrgAddress: testOrgAddress,
			TwoFaFields: OrgMemberTwoFaFields{
		...
		participants3, err := testDB.CensusParticipants(censusOID3)
		c.Assert(err, qt.IsNil)
