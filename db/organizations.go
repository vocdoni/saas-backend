...
// This method uses MongoDB's atomic $inc operator to avoid race conditions when multiple concurrent operations
// try to update the same counter.
...
// This method uses MongoDB's atomic $inc operator with a negative value
// to avoid race conditions when multiple concurrent operations try to update the same counter.
