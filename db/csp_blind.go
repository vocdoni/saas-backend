...
type CSPBlindSession struct {
	...
	// Salt is the fork-aware CSP salt derived for this election and authorized
	// weight when the session was prepared, stored so the matching sign call
	// blinds under the exact key the weight attestation was made over. It is
	// fixed here rather than re-derived so the two requests cannot disagree.
...
// SetCSPBlindSession stores the ephemeral secret k and the fork-aware salt for a
// voter's blind-signature session, replacing any session that voter already had
// open on the same election. Replacing is intentional: preparing again hands the
// client a fresh R point, and the previous one must stop being signable.
func (ms *MongoStorage) SetCSPBlindSession(
	userID, electionID internal.HexBytes, secretK, tokenR, salt []byte, ttl time.Duration,
) error {
