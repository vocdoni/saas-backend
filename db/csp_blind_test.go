...
var (
	...
	testBlindSalt       = []byte("0123456789abcdef0123")
)
...
func TestCSPBlindSessionRoundTrip(t *testing.T) {
	...
	c.Run("bad inputs", func(c *qt.C) {
		c.Assert(testDB.SetCSPBlindSession(
			nil, testBlindElectionID, testBlindSecretK, testBlindTokenR, testBlindSalt, 0,
		), qt.ErrorIs, ErrBadInputs)
		c.Assert(testDB.SetCSPBlindSession(
			testBlindUserID, nil, testBlindSecretK, testBlindTokenR, testBlindSalt, 0,
		), qt.ErrorIs, ErrBadInputs)
		c.Assert(testDB.SetCSPBlindSession(
			testBlindUserID, testBlindElectionID, nil, testBlindTokenR, testBlindSalt, 0,
		), qt.ErrorIs, ErrBadInputs)
	...
	c.Run("store and consume", func(c *qt.C) {
		c.Cleanup(func() { c.Assert(testDB.DeleteAllDocuments(), qt.IsNil) })
		c.Assert(testDB.SetCSPBlindSession(
			testBlindUserID, testBlindElectionID, testBlindSecretK, testBlindTokenR, testBlindSalt, 0,
		), qt.IsNil)
		...
		c.Assert(session.Salt, qt.DeepEquals, testBlindSalt)
	...
	c.Run("expired session is not returned", func(c *qt.C) {
		...
		c.Cleanup(func() { c.Assert(testDB.DeleteAllDocuments(), qt.IsNil) })
		c.Assert(testDB.SetCSPBlindSession(
			testBlindUserID, testBlindElectionID, testBlindSecretK, testBlindTokenR, testBlindSalt, time.Millisecond), qt.IsNil)
	...
	c.Run("preparing again replaces the previous session", func(c *qt.C) {
		...
		c.Assert(testDB.SetCSPBlindSession(testBlindUserID, testBlindElectionID, newK, testBlindTokenR, testBlindSalt, 0), qt.IsNil)
