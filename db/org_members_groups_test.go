...
func TestOrganizationMemberGroup(t *testing.T) {
	...
	t.Run("CreateOrganizationMembersGroup", func(_ *testing.T) {
		...
		t.Run("InvalidData", func(_ *testing.T) {
			...
			_, err := testDB.CreateOrganizationMembersGroup(invalidGroup)
			...
			_, err = testDB.CreateOrganizationMembersGroup(invalidMemberGroup)
			c.Assert(err, qt.Not(qt.IsNil))
		})
		...
		t.Run("ValidGroup", func(_ *testing.T) {
			...
			groupID, err := testDB.CreateOrganizationMembersGroup(validGroup)
	...
	t.Run("OrganizationMemberGroup", func(_ *testing.T) {
		...
		t.Run("ExistingGroup", func(_ *testing.T) {
			...
			groupID, err := testDB.CreateOrganizationMembersGroup(group)
			c.Assert(err, qt.IsNil)
	...
	t.Run("OrganizationMemberGroups", func(_ *testing.T) {
		...
		t.Run("EmptyList", func(_ *testing.T) {
			...
			groups, err := testDB.OrganizationMemberGroups(testOrgAddress)
			c.Assert(err, qt.IsNil)
		...
		t.Run("MultipleGroups", func(_ *testing.T) {
			...
			for i := 0; i < 3; i++ {
				...
				_, err := testDB.CreateOrganizationMembersGroup(group)
				c.Assert(err, qt.IsNil)
			}
		...
		t.Run("DifferentOrganizations", func(_ *testing.T) {
			...
			groups1, err := testDB.OrganizationMemberGroups(testOrgAddress)
			c.Assert(err, qt.IsNil)
			...
			groups2, err := testDB.OrganizationMemberGroups("different_org")
			c.Assert(err, qt.IsNil)
	...
	t.Run("UpdateOrganizationMembersGroup", func(_ *testing.T) {
		c.Assert(testDB.Reset(), qt.IsNil)
		// Setup prerequisites
		...
		t.Run("NonExistentGroup", func(_ *testing.T) {
			...
			nonExistentID := primitive.NewObjectID().Hex()
			err := testDB.UpdateOrganizationMembersGroup(
