...
func TestOrganizationMemberGroup(t *testing.T) {
	...
	t.Run("CheckGroupMembersFields", func(_ *testing.T) {
		...
		c.Assert(len(results.Members) < 2, qt.IsTrue) // Not all members are in the Members field

		// Should find duplicates (member1 and member3 have same name+surname+memberNumber)
		c.Assert(len(results.Duplicates) >= 2, qt.IsTrue)

		// Check that member1 and member3 IDs are in the duplicates list
		c.Assert(contains(results.Duplicates, member1ID) && contains(results.Duplicates, member3ID), qt.IsTrue)
		...
		c.Assert(len(results.MissingData) > 0, qt.IsTrue)

		// Check that member4 ID is in the missing data list
		c.Assert(contains(results.MissingData, member4ID), qt.IsTrue)
		...
		c.Assert(len(results.Members) > 0, qt.IsTrue) // At least some members should be in the Members field
		// Check for duplicates and missing data
		c.Assert(len(results.Duplicates) >= 0, qt.IsTrue)  // May or may not have duplicates
		c.Assert(len(results.MissingData) >= 0, qt.IsTrue) // May or may not have missing data
		...
		c.Assert(err, qt.IsNil)
		c.Assert(results, qt.Not(qt.IsNil))
		c.Assert(len(results.Members) >= 0, qt.IsTrue)     // May or may not have members
		c.Assert(len(results.Duplicates) >= 0, qt.IsTrue)  // May or may not have duplicates
		c.Assert(len(results.MissingData) >= 0, qt.IsTrue) // May or may not have missing data
		...
		emptyPhoneGroup := &OrganizationMemberGroup{
			...
			MemberIDs:   []internal.ObjectID{emptyPhoneMemberID, dupPhone1MemberID, dupPhone2MemberID},
		}
		...
		c.Assert(contains(results.MissingData, emptyPhoneMemberID), qt.IsTrue)
		...
		twoFaGroup := &OrganizationMemberGroup{
			...
			MemberIDs:   []internal.ObjectID{onlyEmailMemberID, onlyPhoneMemberID, bothMemberID, noneMemberID},
		}
		...
		c.Assert(contains(results.Members, onlyEmailMemberID), qt.IsTrue)
		c.Assert(contains(results.Members, onlyPhoneMemberID), qt.IsTrue)
		c.Assert(contains(results.Members, bothMemberID), qt.IsTrue)

		// Member with neither email nor phone should be in missing data
		c.Assert(contains(results.MissingData, noneMemberID), qt.IsTrue)
		...
		sameAuthGroup := &OrganizationMemberGroup{
			...
			MemberIDs:   []internal.ObjectID{sameAuth1ID, sameAuth2ID},
		}
