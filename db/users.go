...
func (ms *MongoStorage) addOrganizationToUser(ctx context.Context,
	userEmail string, address internal.HexBytes, role UserRole,
) error {
...
func (ms *MongoStorage) UserHasRoleInOrg(userEmail string, organizationAddress internal.HexBytes, role UserRole) (bool, error) {
	user, err := ms.UserByEmail(userEmail)
	if err != nil {
		return false, err
	}
	for _, org := range user.Organizations {
		if org.Address.Equals(organizationAddress) {
...
// occurs, it returns the error.
func (ms *MongoStorage) UserHasAnyRoleInOrg(userEmail string, organizationAddress internal.HexBytes) (bool, error) {
	user, err := ms.UserByEmail(userEmail)
	if err != nil {
		return false, err
	}
