	...
	t.Run("Load", func(_ *testing.T) {
		...
		for _, templateFile := range expectedTemplates {
			for _, lang := range []string{"en", "es", "ca"} {
				_, exists := available[TemplateFile(string(templateFile)+"_"+lang)]
...
func TestVerifyAccountNotification(t *testing.T) {
	...
	t.Run("TemplateStructure", func(_ *testing.T) {
		...
		c.Assert(template.PlainText.Subject, qt.Equals, "Verify Your Vocdoni Account")
		c.Assert(template.PlainText.Body, qt.Contains, "Your Vocdoni verification code is:")
		c.Assert(template.PlainText.Body, qt.Contains, "{{.Code}}")
		c.Assert(template.PlainText.Body, qt.Contains, "{{.Link}}")
	})
...
func TestPasswordResetNotification(t *testing.T) {
	...
	t.Run("TemplateStructure", func(_ *testing.T) {
		...
		// Test template field values
		c.Assert(template.PlainText.Subject, qt.Equals, "Vocdoni password reset")
		c.Assert(template.PlainText.Body, qt.Contains, "Your Vocdoni password reset code is:")
		c.Assert(template.PlainText.Body, qt.Contains, "{{.Code}}")
		c.Assert(template.PlainText.Body, qt.Contains, "{{.Link}}")
	})
...
func TestInviteNotification(t *testing.T) {
	...
	t.Run("TemplateStructure", func(_ *testing.T) {
		...
		// Test template field values
		c.Assert(template.PlainText.Subject, qt.Equals, "Vocdoni organization invitation")
		c.Assert(template.PlainText.Body, qt.Contains, "Your code to join '{{.Organization}}' organization is:")
		c.Assert(template.PlainText.Body, qt.Contains, "{{.Code}}")
		c.Assert(template.PlainText.Body, qt.Contains, "{{.Link}}")
	})
...
func TestSupportNotification(t *testing.T) {
	...
	t.Run("TemplateStructure", func(_ *testing.T) {
		...
		// Test template field values
		c.Assert(template.PlainText.Subject, qt.Contains, "New {{.Type}} Ticket from {{.Email}}: {{.Title}}")
		c.Assert(template.PlainText.Body, qt.Contains, "You have a new support request:")
		c.Assert(template.PlainText.Body, qt.Contains, "{{.Title}}")
		c.Assert(template.PlainText.Body, qt.Contains, "{{.Type}}")
		c.Assert(template.PlainText.Body, qt.Contains, "{{.Email}}")
		c.Assert(template.PlainText.Body, qt.Contains, "{{.Organization}}")
		c.Assert(template.PlainText.Body, qt.Contains, "{{.Description}}")
	})
...
func TestMembersImportCompletionNotification(t *testing.T) {
	...
	t.Run("TemplateStructure", func(_ *testing.T) {
		...
		// Test template field values
		c.Assert(template.PlainText.Subject, qt.Contains, "Members import completed for {{.OrganizationName}}")
		c.Assert(template.PlainText.Body, qt.Contains, "Hello {{.UserName}}")
		c.Assert(template.PlainText.Body, qt.Contains, "{{.TotalMembers}}")
		c.Assert(template.PlainText.Body, qt.Contains, "{{.AddedMembers}}")
		c.Assert(template.PlainText.Body, qt.Contains, "{{.ErrorCount}}")
		c.Assert(template.PlainText.Body, qt.Contains, "{{.CompletedAt}}")
	})
...
func TestVerifyOTPCodeNotification(t *testing.T) {
	...
	t.Run("TemplateStructure", func(_ *testing.T) {
		...
		// Test template field values
		c.Assert(template.PlainText.Subject, qt.Equals, "Codi de Verificació - Vocdoni")
		c.Assert(template.PlainText.Body, qt.Contains, "El teu codi de verificació és:")
		c.Assert(template.PlainText.Body, qt.Contains, "{{.Code}}")
	})
...
func TestMailTemplateExecution(t *testing.T) {
	...
	t.Run("ExecPlain_EmptyPlaceholder", func(_ *testing.T) {
		...
		template := LocalizedTemplate{
			HTMLFile:  "assets/verification_account_en.html",
			PlainText: TemplatePlainText{},
		}
