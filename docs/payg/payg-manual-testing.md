...
- **Paid draft is editable**: once paid, `PUT /processes/{id}` still works — the publish
  ...
  refused (409) is deleting it, editing or deleting while a payment is `processing`, or
  deleting one whose Stripe session you just completed but whose webhook has not landed.
