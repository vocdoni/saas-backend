...
func (p *Subscriptions) OrgCanAddNCenusParticipants(orgAddress common.Address, censusID string, pariticipantsNumber int) error {
	// Check if the organization has a subscription
	org, err := p.db.Organization(orgAddress)
	if err != nil {
		return errors.ErrOrganizationNotFound.WithErr(err)
	...
	if int(count)+pariticipantsNumber > plan.Organization.MaxCensus {
